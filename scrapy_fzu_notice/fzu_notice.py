...
if __name__=="__main__":
    ...
    cur=db.cursor()
    sum=notice_num_need=int(input("你需要的消息数量"))#你需要的消息数
                ...
                if len(page_notice_list)<=notice_num_need:
                    ...
                    for one_notice in page_notice_list:#一次执行一次数据库插入
                        ...
                        if re.match(r'.*?(info/\d+/\d+.htm).*?',one_notice.xpath("./a/@href")[0])  :
                            ...
                            #attatchment_link=paqu_fujian(notice_link)[0]#附件信息的元组，包括下载地址，标题，下载次数
                            #attatchment_title=paqu_fujian(notice_link)[1]
                            #attatchment_times=paqu_fujian(notice_link)[2]
                            ...
                            #for i in paqu_fujian(notice_link):
                               # cur.execute(insert_execute_fujian%(i))
